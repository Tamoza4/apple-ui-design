...
description: Generate Apple‑style user interfaces with high precision, strictly in Light Mode. Use this skill when the user requests interfaces that mimic Apple’s design language (iOS, macOS, visionOS) – websites, components, dashboards, or applications. Delivers production‑ready code that embodies clarity, deference, and depth, regardless of framework (HTML/CSS, React, Vue, SwiftUI, Flutter, etc.).
---

This skill guides creation of interfaces that mirror Apple’s elite UI aesthetics with pixel‑perfect accuracy, using Light Mode exclusively. Every output must feel native to Apple’s ecosystem, be fully functional, and follow the rules below.
...
**CRITICAL**: Work exclusively in Light Mode. No dark mode, no alternative themes. The palette is crisp, warm‑gray‑based, and uses translucency to build visual layers.
...
## What to Avoid
- Using third‑party fonts like Inter, Roboto, Helvetica, or Arial in the font stack.
- Solid black borders (`#000000`). Borders must be translucent or light gray.
- Dark mode or any dark color scheme.
- Dimensions that are not multiples of 8px (except for minor fine‑tuning as allowed above).
...
2. **Light Mode Only** – No dark mode, no dark color schemes.
3. **Typography** – Uses Apple’s system fonts as the primary; no generic fallbacks (Inter, Roboto, etc.) appear in the font stack.
...
6. **Borders** – All borders are translucent or light gray; no solid black borders.
...
`Apple UI Design System – Light Mode – Verified: 8pt, glassmorphism, SF fonts, natural motion`
